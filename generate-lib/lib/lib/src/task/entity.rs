...
impl Task {
    ...
    pub fn match_field(&self) -> String {
        self.changed_match_field().and_then(|value| value.try_text().map(|value| value.to_owned())).unwrap_or_else(|| self.match_field.clone())
    }

    pub fn update_match_field(&mut self, value: impl Into<teaql_core::Value>) -> &mut Self {
        let value = value.into();
        self.match_field = value.try_text().map(|value| value.to_owned()).unwrap_or_else(|| self.match_field.clone());
        self.root.set(self.entity_key(), "match", value);
        self
    }

    pub fn changed_match_field(&self) -> Option<teaql_core::Value> {
        self.root.get(&self.entity_key(), "match")
    }
