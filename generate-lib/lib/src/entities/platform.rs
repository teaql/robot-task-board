...
pub struct Platform {
    ...
    pub task_statuses: Vec<crate::entities::task_status::TaskStatus>,
...
impl Platform {
    ...
    pub fn task_statuses_mut(&mut self) -> &mut Vec<crate::entities::task_status::TaskStatus> {
        &mut self.task_statuses
    }
    ...
    pub fn update_user_email(&mut self, value: impl Into<String>) -> &mut Self {
    ...
    pub fn update_session_id(&mut self, value: impl Into<String>) -> &mut Self {
    ...
    pub fn save(mut self, ctx: &teaql_runtime::UserContext) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<teaql_runtime::GraphNode, std::io::Error>> + Send + '_>> {
        Box::pin(async move {
            ...
            let mut list = std::mem::take(&mut self.task_statuses);
            for item in list {
                let mut log_node = teaql_runtime::GraphNode::new("task_status");
                if item.deleted {
            ...
            if !items.is_empty() {
                node.relations.insert("task_status_list".to_string(), items);
            }
            let mut items = Vec::new();
        ...
        teaql_core::EntityDescriptor { 
            ...
            relations: vec![
                teaql_core::RelationDescriptor {
                    name: "task_status_list".to_string(),
...
impl teaql_core::Entity for Platform {
    fn from_record(mut record: std::collections::BTreeMap<String, teaql_core::Value>) -> Result<Self, teaql_core::EntityError> {
        ...
        if let Some(val) = record.remove("user_email") {
            if let teaql_core::Value::Text(v) = val { entity.user_email = Some(v); }
        }
        if let Some(val) = record.remove("session_id") {
            if let teaql_core::Value::Text(v) = val { entity.session_id = Some(v); }
        }
        Ok(entity)
    }
    ...
    fn into_record(self) -> std::collections::BTreeMap<String, teaql_core::Value> {
        ...
        if let Some(v) = self.user_email { record.insert("user_email".to_string(), teaql_core::Value::Text(v)); }
        if let Some(v) = self.session_id { record.insert("session_id".to_string(), teaql_core::Value::Text(v)); }
        record
    }
}
