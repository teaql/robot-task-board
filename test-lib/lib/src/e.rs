...
impl E {
    ...
    pub fn task_status<'a>(value: &'a crate::TaskStatus) -> crate::TaskStatusExpression<'a> {
        let root_desc = std::sync::Arc::new(format!("TaskStatus(id={})", value.id()));
    ...
    pub fn task_execution_log<'a>(value: &'a crate::TaskExecutionLog) -> crate::TaskExecutionLogExpression<'a> {
        let root_desc = std::sync::Arc::new(format!("TaskExecutionLog(id={})", value.id()));
        crate::TaskExecutionLogExpression::new(teaql_core::eval::EvalResult::Value(value), root_desc)
    }
}
