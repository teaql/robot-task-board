    ...
    C: TeaqlRuntime + ?Sized,
{
    ...
    load_root_attributes(ctx, &mut state).await?; //depth: 0
    load_root_objects(ctx, &mut state).await?; //depth: 0
...
async fn load_root_attributes<C>(
    ctx: &C,
...
where
    C: TeaqlRuntime + ?Sized,
{
    let list = Q::attributes().execute_for_list(ctx).await.unwrap_or_default();
    for item in list {
        state.add_reference("attribute", item.id().into_u64());
    }
    Ok(())
}

async fn load_root_objects<C>(
    ctx: &C,
    state: &mut SampleDataState,
) -> Result<(), String>
where
    C: TeaqlRuntime + ?Sized,
{
    let list = Q::objects().execute_for_list(ctx).await.unwrap_or_default();
    for item in list {
        state.add_reference("object", item.id().into_u64());
    }
    Ok(())
}
