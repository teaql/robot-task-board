...
pub trait TaskStatusCheckerLogic: Send + Sync {
    fn check_and_fix_task_status(
        &self,
        _ctx: &UserContext,
        _entity: &mut crate::TaskStatus,
...
impl Default for TaskStatusChecker<NoopTaskStatusChecker> {
    fn default() -> Self {
        Self {
            logic: NoopTaskStatusChecker,
        }
    }
}

impl<L> TaskStatusChecker<L>
where
...
impl<L> TypedChecker<crate::TaskStatus> for TaskStatusChecker<L>
where
    L: TaskStatusCheckerLogic,
{
    fn check_and_fix_typed(
        &self,
        ctx: &UserContext,
        entity: &mut crate::TaskStatus,
        status: CheckObjectStatus,
        location: &ObjectLocation,
        results: &mut CheckResults,
    ) {
        self.logic
            .check_and_fix_task_status(ctx, entity, status, location, results);
    }
}