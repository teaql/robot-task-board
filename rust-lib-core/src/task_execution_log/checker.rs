...
pub trait TaskExecutionLogCheckerLogic: Send + Sync {
    fn check_and_fix_task_execution_log(
        &self,
        _ctx: &UserContext,
...
impl Default for TaskExecutionLogChecker<NoopTaskExecutionLogChecker> {
    fn default() -> Self {
        Self {
            logic: NoopTaskExecutionLogChecker,
        }
    }
}

impl<L> TaskExecutionLogChecker<L>
where
...
impl<L> TypedChecker<crate::TaskExecutionLog> for TaskExecutionLogChecker<L>
where
    L: TaskExecutionLogCheckerLogic,
{
    fn check_and_fix_typed(
        ...
        self.logic
            .check_and_fix_task_execution_log(ctx, entity, status, location, results);
    }
}