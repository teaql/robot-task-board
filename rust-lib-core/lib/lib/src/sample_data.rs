    ...
    C: TeaqlRuntime + ?Sized + crate::TeaqlRepositoryProvider,
{
    ...
    for item in list {
        state.add_reference(crate::TaskStatus::ENTITY_NAME, item.id().into_u64());
    }
    Ok(())
}
    ...
    C: TeaqlRuntime + ?Sized + crate::TeaqlRepositoryProvider,
{
        if state.ids("Task Status").is_empty() {
            state.record_skipped(crate::Task::ENTITY_NAME, "Required dependency Task Status is missing in reference pool".to_string());
        ...
        if state.ids("Platform").is_empty() {
            state.record_skipped(crate::Task::ENTITY_NAME, "Required dependency Platform is missing in reference pool".to_string());
    ...
    for i in 0..fanout {
        ...
        state.record_generated(crate::Task::ENTITY_NAME);

        if i % 20 == 0 {
            log::info!("Generating Task: {}/{}", i, fanout);
        }

        state.add_reference(crate::Task::ENTITY_NAME, entity.id().into_u64());
    }
    ...
    C: TeaqlRuntime + ?Sized + crate::TeaqlRepositoryProvider,
{
        if state.ids("Task").is_empty() {
            state.record_skipped(crate::TaskExecutionLog::ENTITY_NAME, "Required dependency Task is missing in reference pool".to_string());
...
entity.audit_as("Init Sample Data").save(ctx).await.map_err(|e| e.to_string())?;
        ...
        state.record_generated(crate::TaskExecutionLog::ENTITY_NAME);
